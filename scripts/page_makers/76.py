...
def get_output(file_number):
    output = ''
    total_lines = randint(4,9)
    random_line = randint(0, total_lines)
    header_line = ''
    while len(header_line) < 48:
        header_line += word_sets['upper'][file_number] + " " 
    header_length = len(header_line)
    header_remainder = header_length - 43
    padding = int(header_remainder / 2)
    # padding = header_remainder(header_remainder / 2)
    for i in range(0,random_line):
        output += header_line
        output += "\n"
    output += " " * padding
    output += straight(case='upper')
    for i in range(random_line, total_lines):
        output += header_line
        output += "\n"
