...
def get_output(file_number):
    output = ''
    total_lines = randint(2,5)
    output_line = randint(0,total_lines)
    for i in range(0, output_line):
        output += only_letters(letters=[letter_sets['lower_all'][file_number]], case='lower')
        output += "\n"
    output += letters_removed(letters=[letter_sets['lower_all'][file_number]], case='lower')
    output += "\n"
...
if __name__ == '__main__':
    ...
    for word in word_sets['lower']:
        for letter in word:
            basic_set.add(letter)
    letter_sets['lower_all'] = list(basic_set)
