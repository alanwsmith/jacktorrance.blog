...
def get_output(file_number):
    ...
    for i in range(0, 3):
        line = ""
        while len(line) < 51:
            line += word_sets['lower'][randint(0,9)]
            if len(line) > 51:
                line = ''
        lines.append(line)
    ...
    preface = ''
    while len(preface) < 8:
        preface += word_sets['lower'][randint(0,9)]
        if len(preface) > 8:
            preface = ''
    afterword = ''
    while len(afterword) < 8:
        afterword += word_sets['lower'][randint(0,9)]
        if len(afterword) > 8:
            afterword = ''

    lines.append(preface + ' ' + ''.join(word_sets['upper']) + ' ' + afterword)
