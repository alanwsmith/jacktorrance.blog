...
def get_output(file_number):
    output = ''
    top_words = [x for x in range(0,9,2)]
    bottom_words = [x for x in range(1,10,2)]
    for i in range(0,3):    
        output += word_at_positions(positions=bottom_words, case='upper')
        output += "\n"
        output += word_at_positions(positions=top_words, case='upper')
        output += "\n"
    return output
