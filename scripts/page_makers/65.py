...
def get_output(file_number):
    output = ''
    target_row = randint(0, 7)
    target_col = randint(1, 8)
    word_indexes = list(range(0,10))
    word_indexes.remove(target_col)
    for i in range(0, 7):
        if i == target_row:
            output += words_at_positions(positions=word_indexes, case="upper")
            output += "\n"
        else:
