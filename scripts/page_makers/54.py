...
def only_letters(*, letters=[], case='default'):
    return_list = []
    for word in word_sets[case]:
        updated_word = re.sub(r'[^' + ''.join(letters) + ']' , ' ', word)
        return_list.append(updated_word)
    return " ".join(return_list)
...
def get_output(file_number):
    output = ''
    the_letters = []
    for i in range(file_number, file_number + 6):
        the_letters.append(letter_sets['lower_all'][i])
    output += letters_removed(letters=the_letters, case='lower')
    output += "\n"
    output += only_letters(letters=the_letters, case='lower')
    return output
...
if __name__ == '__main__':
    ...
    for file_number in range(0,len(letter_sets['lower_all']) - 7):
