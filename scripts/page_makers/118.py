...
def get_output(file_number):
    ...
    for top_index in range(0, random_line):
        line = ""
        for letter_index in range(0, 33):
            line += letters[randint(0, len(letters) - 1)]
        lines.append(line)
    for add_line in range(0, 3):
        lines.append("".join(word_sets['upper']))
    for bottom_index in range(random_line, total_lines):
        line = ""
        for letter_index in range(0, 33):
            line += letters[randint(0, len(letters) - 1)]
        lines.append(line)
